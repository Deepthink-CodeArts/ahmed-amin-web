...
# Ahmed Amin Group - Fixed Docker Deployment Script
echo "🐳 Deploying with Fixed Docker to ahmedamin.deepthinkcodearts.com..."
...
print_step "2. Building Docker image (trying Alpine first)..."
cd frontend

# Try the fixed Alpine Dockerfile first
docker build -t ahmed-amin-frontend .
if [ $? -ne 0 ]; then
    print_warning "Alpine build failed, trying Ubuntu alternative..."
    
    # Try the alternative Ubuntu Dockerfile
...
print_status "🎉 Fixed Docker deployment completed successfully!"
